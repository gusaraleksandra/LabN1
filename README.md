...
| B (Число 3) | Целое положительное (натуральное число)  | 1               | -              |
| C (Число 4) | Целое положительное (натуральное число)  | 1               | -              |
...
6. **Третий покупатель:**  
   Программа сравнивает значения `x` и `c`. Если `x` больше или равно `c`, программа увеличивает переменную `count` на 1 и уменьшает значение `x` на `c`(новое `x` становится равно `x-c`. Если `x` меньше `с` переходим к выводу ответа.
...
2. Тесты на `Купили два покупателя из трех`:
    ...
    - **Output**:
        ...
        2
        ```
3. Тест на `Купил только один покупатель`:
